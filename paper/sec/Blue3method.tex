...
\begin{figure*}
    ...
    \textcolor{blue}{\textbf{A. Predict Stage:} Both historical context and target future frames are encoded into continuous latent pyramids via a pre-trained VAE followed by asymmetric downsampling. An autoregressive temporal planner aggregates historical features to generate multi-scale semantic representations $\{S^1, S^2, \dots\}$.
...
\textcolor{blue}{To circumvent this, we construct a continuous pyramidal latent representation to map visual observations onto a smooth, hierarchical, and differentiable manifold. 
...
\textcolor{blue}{To establish the multi-scale hierarchy required for coarse-to-fine generation, we introduce a recursive downsampling operator $\mathcal{P}_{\downarrow}$, inspired by Laplacian pyramids~\cite{denton2015deep}. The resulting latent pyramid $\mathcal{Z}_t = \{ \boldsymbol{z}_t^1, \boldsymbol{z}_t^2, \dots, \boldsymbol{z}_t^S \}$ is defined as:
\begin{equation}
...
\noindent \textcolor{blue}{\textit{\textbf{1) Inter-frame Temporal Autoregression: The Causal Planner.}}
...
\textcolor{blue}{Within each Transformer block, this active modulation is applied to the intermediate latent feature representation $\boldsymbol{u}$ prior to the self-attention and Feed-Forward Network (FFN) modules. Consistent with our architectural modifications detailed in Section~\ref{Sec3:UnifiedFramework}, we employ RMSNorm to stabilize the continuous flow integration:
\begin{equation}
